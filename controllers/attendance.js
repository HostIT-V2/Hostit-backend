...
const Attendance = require("../models/attendance"),
  ...
  aw3cEventAbi = require("../abis/aw3cAbi.json"),
  bdrlsEventAbi = require("../abis/bdrlsAbi.json"),
  fs = require("fs");
...
exports.getAttendeeDetails = async (req, res) => {
  ...
  try {
    ...
    if (attendeeResult.length == 1 && speakerResult.length == 1) {
      ...
      return res
        .status(200)
        .json({ message: "success", attendee: attendeeResult });
    } else if (speakerResult.length == 1) {
      return res
        .status(200)
        .json({ message: "success", speaker: speakerResult });
    } else {
    ...
    return res
      .status(400)
      .json({
        message: "failed to get attendee details",
        error: error.message,
      });
  }
};
...
exports.verifyAttendance = async (req, res) => {
  ...
  try {
    ...
    const eventContract = new ethers.Contract(
      process.env.LISK_CONTRACT_ADDRESS,
      eventAbi,
      wallet
    );
    ...
    if (!foundUser) {
      ...
      // mark attendance
      const markAttTx = await eventContract.w3lc2024__markAttendance(
      ...
      const markAtt = await Attendance.create({
        email: email,
        isPresent: true,
        day: day,
      });
      ...
      const transaction = await eventContract.w3lc2024__markAttendance(
        foundUser.address,
        day
      );
      ...
      let response = await Attendance.create({
        email: email,
        isPresent: true,
        day: day,
      });
    ...
    return res
      .status(400)
      .json({ message: "failed to verify", error: error.message });
  }
};

// 
exports.verifyAttendanceAW3C = async (req, res) => {
  let { email, day } = req.body;
  ...
  try {
    ...
    // get smart contract instance with ethers
    const aw3cEventContract = new ethers.Contract(
      process.env.LISK_CONTRACT_ADDRESS,
      aw3cEventAbi,
      wallet
    );
    ...
    try {
      await fetch(
        `${process.env.LUMA_URL}?event_api_id=${
          process.env.LUMA_AW3C_ID
        }&email=${encodeURIComponent(email)}`,
        {
          method: "GET",
          headers: {
            "x-luma-api-key": `${process.env.LUMA_TOKEN}`,
    ...
    if (!foundUser) {
      ...
      // mark attendance
      const markAttTx = await aw3cEventContract.aw3c2024__markAttendance(
        data.user.walletPublicKey,
        day
      );
      ...
      const transaction = await aw3cEventContract.aw3c2024__markAttendance(
        foundUser.address,
        day
      );
...
exports.verifyAttendanceBDRLS = async (req, res) => {
  let { email, day } = req.body;
  ...
  try {
    ...
    // get smart contract instance with ethers
    const bdrlsEventContract = new ethers.Contract(
      process.env.LISK_CONTRACT_ADDRESS,
      bdrlsEventAbi,
      wallet
    );
    ...
    if (!foundUser) {
      ...
      // mark attendance
      const markAttTx = await bdrlsEventContract.bdrls2024__markAttendance(
        data.user.walletPublicKey,
        day
      );
      ...
      const transaction = await bdrlsEventContract.bdrls2024__markAttendance(
        foundUser.address,
        day
      );
...
exports.markAttendance = async (req, res) => {
  ...
  try {
    ...
    const transaction = await contract.w3lc2024__markAttendance(
    ...
    let foundCheckin = await Attendance.findOne({
      email,
      createdAt: {
        $gte: targetDate,
...
exports.distributePOAP = async (req, res) => {
  try {
    ...
    for (const user of users) {
      await VerifiedUser.sendEmail(
        user.email,
        "WEB3 lagos conf. 2024 POAP",
        "poap"
      );
      console.log("done");
    }
    ...
    return res
      .status(400)
      .json({ message: "failed to distribute POAP", error: error.message });
  }
};
...
exports.getAllAttendance = async (req, res) => {
  try {
    ...
    return res
      .status(400)
      .json({ message: "failed to get all Attendance", error: error.message });
  }
};
